...
```{r}
# Vil ha ut tallene 8, 14 og 17 i en ny verktor m
(m <- z[c(2, 5:6)])
```
...
```{r}
# Vil ha ut tallene 8, 14 og 17 og legge til første element og legge ut resultatet inn i vektor m
(m <- z[c(2, 5:6)] + z[1])
```
...
```{r}
sex_f <- fct(x = as.character(sex), levels = c("1", "2"))
```

```{r}
# new labels, "F" for 1 and "M" for 2
sex_f <- fct(x = as.character(sex), recode("1" = "Kvinner", "2" = "Menn")
```
...
```{r}
lm_cars <- lm(formula = 'dist ~ speed', data = cars)
```

```{r}
lm_cars_subset <- lm(formula = 'dist ~ speed', data = cars_sub)
```
...
```{r}
plot(x = mtcars$disp, y = mtcars$mpg, # graphics parameter
           pch = 11)
